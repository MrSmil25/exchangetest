  ...
  return (
    <div className="relative mx-auto aspect-square w-full max-w-[620px] animate-exchange-rise [animation-delay:120ms]">
      ...
      <div className="absolute bottom-[2%] left-1/2 w-[72%] -translate-x-1/2 rounded-lg border border-glass-border bg-glass-strong p-4 shadow-glass backdrop-blur-2xl">
        <div className="flex items-center justify-between gap-4"><div><p className="text-[10px] uppercase text-muted-foreground">Network signal</p><p className="mt-1 text-sm font-semibold">Your identity strengthens with every exchange</p></div><span className="font-num text-2xl font-bold text-signal">+12</span></div>
      </div>
    </div>
  );
}
...
function PassportSkill({ label, value }: { label: string; value: string }) { return <div><div className="mb-2 flex justify-between text-xs"><span>{label}</span><span className="font-num text-signal">{value}/100</span></div><div className="h-1 overflow-hidden rounded-full bg-glass-surface"><div className="h-full rounded-full bg-aurora-line animate-passport-fill" style={{ width: `${value}%` }} /></div></div>; }